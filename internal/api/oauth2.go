...
func (o *Oauth2Api) Credentials(c *gin.Context) {
	...
	timestamp := time.Now().UnixNano() / int64(time.Millisecond)
	appUrl := fmt.Sprint("http://103.225.11.21:10008/auth/code?client_id=", clientID, "&redirect_uri=", client.GetCallbackUrl(), "&response_type=token&scope=all&state=", timestamp)
